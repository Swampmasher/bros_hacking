...
TogglePower = function()
{
	if (Power == "on")
	{
		...
		$("#terminal").html("");
		$.post('http://bros-fallouthacker/CloseMenuv2', JSON.stringify({suces: null}));
		if (Sound)
			$("#poweroff")[0].play();
	}
	else
...
HandleBraces = function(DudCap)
{
	...
	else
	{
		...
		$.post('http://bros-fallouthacker/bildirim', JSON.stringify({}));
		RemoveDud();
	}
...
Failure = function()
{
	...
	},
	{
		...
		complete : function()
		{
			$("#terminal").html("<div id='canvas'></div><div id='adminalert'><div class='character-hover alert-text' id = 'hatali'>HACKING FAILURE CLICK TO CONTINUE!</div></div>");
			...
			function hatali() {
				$.post('http://bros-fallouthacker/CloseMenu', JSON.stringify({suces: false}));
				TogglePower()
			}
...
Success = function()
{
	...
	},
	{
		...
		complete : function()
		{
			...
			$("#terminal").html("<div id='canvas'></div><div id='adminalert'><div id='msg' class='character-hover alert-text'>HACKING SUCCESSFUL</div><br /><div id='proceed' class='alert-text'>CLICK TO CONTINUE!</div></div>");
			...
			document.getElementById("proceed").onclick = suces;

			function suces() {
				$.post('http://bros-fallouthacker/CloseMenu', JSON.stringify({suces: true}));
				TogglePower()
			}
